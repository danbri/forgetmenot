...
// ---------------------------------------------------------------
// 5b. AI opt-out signals (llms.txt, ai.txt, noai meta)
// ---------------------------------------------------------------
//
// A growing set of conventions lets sites declare a separate stance
// for AI tools, distinct from the search-engine-oriented robots.txt:
//
//   * `/llms.txt` — markdown-format guide proposed at
//     https://llmstxt.org for LLM-friendly summaries of a site.
//     Has no normative "block" semantics. We fetch and record it
//     because (a) it's polite to read what the publisher wrote
//     for tools like us and (b) downstream consumers may want to
//     prefer the publisher-curated summary over our scraped pages.
//
//   * `/ai.txt` — proposed at https://site.spawning.ai/ for opt-out
//     of AI training. Format mirrors robots.txt with `User-Agent:`
//     and `Disallow:` lines. We fetch and parse it.
//
//   * `<meta name="robots" content="noai, noimageai">` — emerging
//     in-page tag, mirrored by `X-Robots-Tag: noai` HTTP header.
//
// **Position taken by this crawler.** This tool is a user-controlled
// research/archival utility. It does NOT train models on the
// material. Following the practice of archive.org / IA, we
//   * always FETCH and RECORD llms.txt, ai.txt and noai signals
//     so a reviewer can see the publisher's stated wishes;
//   * by default DO NOT use them to refuse fetching, because the
//     fetch is for archival not training;
//   * support a `strictAiOptOut: true` ctx flag for operators who
//     prefer the stricter interpretation (fetch is skipped when
//     ai.txt disallows the path or noai is present).
//
// This is documented and reviewable; flip the flag if you disagree.
export function parseAiTxt(text) {
  // ai.txt follows robots.txt structure. Reuse parseRobots and
  // re-shape the result so callers can distinguish AI-specific
  // disallow from generic robots-disallow.
  const r = parseRobots(text);
  return { allow: r.allow, disallow: r.disallow, raw: String(text || '') };
}

// Parse the <meta name="robots" content="..."> tags from a page
// body and return any `noai`/`noimageai` hints. Also accepts
// `X-Robots-Tag` header values via the optional second arg.
export function parseNoAi(html, xRobotsTag = '') {
  const tags = [];
  const re = /<meta\b[^>]*\bname=["']robots["'][^>]*\bcontent=["']([^"']+)["']/gi;
  let m;
  while ((m = re.exec(String(html || ''))) !== null) {
    tags.push(...m[1].toLowerCase().split(/[,\s]+/).filter(Boolean));
  }
  if (xRobotsTag) tags.push(...String(xRobotsTag).toLowerCase().split(/[,\s]+/).filter(Boolean));
  const flags = {
    noai:      tags.includes('noai'),
    noimageai: tags.includes('noimageai'),
...
async function politeFetch(url, pacer, ctx = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      const eheaders = e instanceof HttpError ? e.headers : null;
      const ebody    = e instanceof HttpError ? e.body    : null;
      if (attempt === 0 && status >= 500 && status < 600) {
        await new Promise((r) => setTimeout(r, 4000));
        continue;
      }
      return { ok: false, url, status, message: e.message, headers: eheaders, body: ebody };
    }
  }
...
//
// Uses the same word-boundary rule as classifyLink (matchesNeedle,
// declared lower in this file) so that `son` does not match
// `lesson`, `season`, `comparison`, `parson`, etc. The smoke run
// against 436 live sites showed `son` firing 111 times under a
// naive substring match — almost all of which were political pages
// like `/lessons-from-x` and `/comparison-of-y`. Word-boundary
// matching makes the exclusion specific enough to do its actual
// job (skip pages about a named family member) without bulk-
// dropping legitimate political content.
export function excludeReason(url) {
  ...
  for (const [needle, why] of EXCLUDE_PERSONAL) {
    if (matchesNeedle(path, needle)) return { rule: needle, reason: why };
  }
  return null;
}
...
export async function crawlSite(member, homepageUrl, opts = {}, ctx = {}) {
  ...
  // 10.1b AI opt-out signals — fetched and recorded for transparency.
  // By default they do NOT block fetching (we are an archival, not a
  // training, crawler). Pass ctx.strictAiOptOut = true to honour them
  // as refusals.
  const aiSignals = { llmsTxt: null, aiTxt: null, applied: false };
  for (const [field, path] of [['llmsTxt', '/llms.txt'], ['aiTxt', '/ai.txt']]) {
    const r = await politeFetch(`${origin}${path}`, pacer, ctx);
    if (r.ok && typeof r.body === 'string' && r.body.length < 200_000) {
      aiSignals[field] = field === 'aiTxt' ? parseAiTxt(r.body) : { raw: r.body };
      log.push({ step: 'ai-optout', file: path, status: 'fetched', bytes: r.body.length });
    } else {
      log.push({ step: 'ai-optout', file: path, status: 'absent', http: r.status });
    }
  }
  ...
  // Live-then-Wayback fetch policy. The wrapped fetch tries the
  // origin first; on any failure (network or HTTP 4xx/5xx) it
  // optionally falls back to the most-recent successful Wayback
  // capture. ctx.waybackFallback === false disables the fallback.
  const wbEnabled = ctx.waybackFallback !== false;
  const tryFetch = (u) => fetchLiveOrWayback(u, pacer, ctx, wbEnabled, log);

  const homeResp = await tryFetch(homepage);
  if (!homeResp.ok || typeof homeResp.body !== 'string') {
    // WAF fingerprinting on the failure. Cheap heuristic against
    // headers + (optionally) the HTML body the origin returned —
    // see lib/waf-detect.mjs for the full pattern list. The result
    // is informational; it does not change crawl behaviour beyond
    // making the manifest auditable.
    let waf = { provider: null, evidence: null, status: homeResp.status };
    try {
      const W = await import('../waf-detect.mjs');
      waf = W.detectWaf(homeResp.status, homeResp.headers, homeResp.body);
    } catch { /* lib not available */ }
    log.push({
      step: 'homepage', status: 'fetch-failed',
      http: homeResp.status, message: homeResp.message,
      waf: waf.provider, waf_evidence: waf.evidence,
    });
    return {
      member, homepageUrl: homepage, origin, ok: false,
      homepage_error: { status: homeResp.status, message: homeResp.message },
      waf_block: waf.provider ? waf : null,
      ai_signals: aiSignals,
      startedAt, finishedAt: new Date().toISOString(), log,
    };
  }
  const homepageHtml = homeResp.body;
  const homeProvenance = homeResp.provenance || 'origin';
  ...
  for (const entry of buckets.values()) {
    ...
    const r = await tryFetch(entry.url);
    if (!r.ok || typeof r.body !== 'string') {
    ...
    pages.push({
      ...
      provenance: r.provenance || 'origin',  // 'origin' | 'wayback:<ts>'
      raw_html: r.body,                      // lossless for downstream review
    });
    log.push({
      step: 'page', url: entry.url, status: 'ok', type: entry.type,
      bytes: r.body.length, provenance: r.provenance || 'origin',
    });
  }
  ...
  return {
    ...
    social,
    ai_signals: aiSignals,
    homepage_provenance: homeProvenance,
...
// Try the origin first; on failure fall back to the most-recent
// successful Wayback Machine capture for the same URL. Records
// the provenance on the returned object.
//
// "Failure" here means network error or HTTP 4xx/5xx. We do NOT
// fall back on robots-disallow at the origin: if the publisher
// asked us not to fetch a path, fetching it from a third-party
// archive would be circumvention. (We fall back on Cloudflare-style
// 403/503 because those aren't expressing publisher intent — they
// are bot-shield heuristics.)
//
// The fallback honours the same ctx.archive sink, so per-fetch
// records are produced for both the failed origin attempt and the
// successful Wayback attempt — useful for auditing.
async function fetchLiveOrWayback(url, pacer, ctx, enabled, log) {
  const live = await politeFetch(url, pacer, ctx);
  if (live.ok && typeof live.body === 'string') {
    return { ...live, provenance: 'origin' };
  }
  if (!enabled) return live;

  // Lazy-load wayback to keep the module lighter when fallback is off.
  let bestCapture, mementoUrl;
  try {
    ({ bestCapture, mementoUrl } = await import('./wayback.mjs'));
  } catch {
    return live;
  }
  ...
  let snap;
  try {
    snap = await bestCapture(url, {}, { ...ctx, retries: 1 });
  } catch (e) {
    log.push({ step: 'wayback', url, status: 'lookup-failed', message: e.message });
    return live;
  }
  if (!snap) {
    log.push({ step: 'wayback', url, status: 'no-capture' });
    return live;
  }

  const mUrl = snap.mementoUrl || mementoUrl(snap.timestamp, url);
  const mResp = await politeFetch(mUrl, pacer, ctx);
  if (!mResp.ok || typeof mResp.body !== 'string') {
    log.push({ step: 'wayback', url, status: 'memento-failed', http: mResp.status, ts: snap.timestamp });
    return live;
  }
  log.push({ step: 'wayback', url, status: 'used', timestamp: snap.timestamp });
  return {
    ok: true,
    status: mResp.status,
    url,                         // the canonical URL we were trying to fetch
    headers: mResp.headers,
    body: mResp.body,
    provenance: `wayback:${snap.timestamp}`,
  };
}
