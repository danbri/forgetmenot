...
export function classifyLink(href, text, originHost) {
  ...
  for (const entry of POLITICAL_TYPES) {
    for (const needle of entry.match) {
      if (path.includes(needle) || t.includes(needle.replace(/-/g, ' '))) {
...
// optionally inside an <a>. We over-generate candidates and let
// the consumer dedupe.
...
// title link and (where present) a date. Date detection uses
// <time datetime=...> first; falls back to plain date strings near
// the title.
