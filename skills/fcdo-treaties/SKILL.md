...
Records are lifted to N-Quads + per-record Turtle under the
project-wide `fm:` namespace (see [`docs/vocab.md`](../../docs/vocab.md)
for the discipline). Outputs land in
...
- [`fm-vocab.ttl`](../../third_party/data/fcdo_treaties/extractors/factoids/fm-vocab.ttl) — formal declaration of every `fm:` class and property emitted by this lift (kept in sync by `scripts/fcdo_treaties_vocab_check.py`)
...
- [`country-qids.tsv`](../../third_party/data/fcdo_treaties/country-qids.tsv) — curated UKTO-label → Wikidata-QID map; extend with a single-PR change, no Python required
...
python3 scripts/fcdo_treaties_vocab_check.py    # asserts script ↔ vocab in sync
```

Blank-node identifiers are deterministic (derived from record id +
predicate kind + index), so re-running produces byte-identical output
and `all.nq.gz` diffs trivially between refreshes.
