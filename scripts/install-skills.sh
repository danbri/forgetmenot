...
# Target path (string) and the symlink text we'd write. Always use
# an absolute path: relative `../../skills/<name>` targets read like
# a path-traversal attempt at a glance, which we'd rather avoid.
# The symlinks are gitignored and regenerated by re-running this
# script, so portability across clones isn't a feature we need.
target_for() {
  local name="$1"
  echo "$SRC_DIR/$name"
}
